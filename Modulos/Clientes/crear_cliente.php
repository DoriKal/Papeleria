            ...
            <td>
            	<?php
					...
					if(!empty($_POST['doc']) and !empty($_POST['nom'])){
							...
							$correo=limpiar($_POST['correo']);	$con=$doc;							
              $url=cadenas().encrypt($doc,'URLCODIGO');  $persona=$_POST['idPersona'];
													
							$oConsultar=new Consultar_Cliente($doc);
							$oAlumno=new Proceso_Cliente($doc,$nom,$ape,$fecha,$tel,$cel,$sexo,$dir,$nota,$fechar,$estado,$correo,$con,$cupo, $persona);
                    	...
                    	<td>
                        	<form name="form1" enctype="multipart/form-data" method="post" action="">
                       		<div class="row-fluid">
                                ...
                                <div class="span4">
                                  <strong>Asignado a Empleado</strong><br>
                                  <select name="idPersona" class="input-xlarge">
                                    <?php
                                    $pa=mysql_query("SELECT * FROM persona");        
                                    while($row=mysql_fetch_array($pa)){
                                      if($row['idPersona']==$persona){
                                        echo '<option value="'.$row['idPersona'].'" selected>'.$row['nom'].'</option>'; 
                                      }else{
                                        echo '<option value="'.$row['idPersona'].'">'.$row['nom'].'</option>';  
                                      }
                                    }
                                    ?>
                                  </select><br>
